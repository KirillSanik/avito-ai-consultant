...
import { EmptyState, ErrorState, Modal, PageLoader, ProgressBar } from "@/components/ui";
import { XlsxImportCard } from "@/components/xlsx";
import { courseApi } from "@/lib/api";
import type { Course, CourseReviewer, HomeworkListItem, Role, User } from "@/lib/types";
...
export function HomeworksScreen({
  ...
  const homeworks = useQuery({
    ...
    queryFn: () => courseApi.homeworks(course.id),
  });
...
function CourseDescriptionForm({ course, canEdit }: { course: Course; canEdit: boolean }) {
  ...
  return (
    <section className="card mb-6 p-5">
      ...
      ) : <p className="whitespace-pre-wrap text-sm leading-6 text-muted">{course.description || "Описание пока не добавлено."}</p>}
    </section>
  );
}
...
function CreateHomeworkModal({
  ...
  const [deadline, setDeadline] = useState("2026-09-20T23:59");
  ...
  return (
    <Modal title="Создание домашнего задания" onClose={onCancel}>
      <form className="grid gap-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); create.mutate(); }}>
        <label className="sm:col-span-2"><span className="field-label">Название</span>
          <input value={title} required minLength={2} onChange={(event) => setTitle(event.target.value)} />
        </label>
        <label className="sm:col-span-2"><span className="field-label">Описание и инструкция</span>
          <textarea rows={4} value={description} required minLength={3} onChange={(event) => setDescription(event.target.value)} />
        </label>
        <label><span className="field-label">Срок сдачи</span>
          <input type="datetime-local" value={deadline} required onChange={(event) => setDeadline(event.target.value)} />
        </label>
        <label><span className="field-label">Ссылка на материал</span>
          <input type="url" value={url} disabled={Boolean(file)} placeholder="https://example.com/task.pdf" onChange={(event) => setUrl(event.target.value)} />
        </label>
        <label className="sm:col-span-2"><span className="field-label">Или загрузите PDF/DOCX</span>
          <input type="file" accept=".pdf,.docx" disabled={Boolean(url)} onChange={(event) => setFile(event.target.files?.[0])} />
        </label>
        {create.error && <p className="sm:col-span-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-danger">{create.error.message}</p>}
        <div className="flex gap-2 sm:col-span-2">
          <button className="button-primary" disabled={create.isPending || (!file && !url)}>{create.isPending ? "Создаём…" : "Создать"}</button>
...
function CreateHomeworkForm({
  ...
  const create = useMutation({
    mutationFn: () =>
      courseApi.createHomework(selectedCourseId, {
        title: form.title,
    ...
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["homeworks", selectedCourseId] }),
        queryClient.invalidateQueries({ queryKey: ["courses"] }),
      ]);
      onCreated();
    },
    ...
    <form className="card mb-6 grid gap-4 p-5 sm:grid-cols-2" onSubmit={submit}>
      ...
      <label className="sm:col-span-2">
        ...
        <select value={selectedCourseId} onChange={(event) => setSelectedCourseId(event.target.value)}>
          {(courses.data ?? []).map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}
        </select>
      </label>
      <label className="sm:col-span-2">
      ...
      <label className="sm:col-span-2">
        <span className="field-label">Описание и инструкция</span>
        <textarea rows={4} value={form.description} required minLength={3}
      ...
      <label>
        <span className="field-label">Ссылка на материал</span>
        <input
          type="url"
          value={form.url}
      ...
      <label className="sm:col-span-2">
        <span className="field-label">Или загрузите материал (PDF/DOCX)</span>
        <input type="file" accept=".pdf,.docx" disabled={Boolean(form.url)}
        ...
        {availableReviewers.length > 0 && (
          <div className="flex flex-wrap items-end gap-2">
            ...
            <button
              ...
              disabled={!pickerId}
              onClick={() => {
                setSelectedReviewerIds((current) => [...current, pickerId]);
                setPickerId("");
              }}
            >
              Добавить
      ...
      <div className="flex flex-wrap gap-2 sm:col-span-2">
        <button className="button-primary" disabled={create.isPending || (!file && !form.url)}>
...
function CourseReviewersPanel({
  ...
  const add = useMutation({
    mutationFn: (userId: string) => courseApi.addReviewer(courseId, userId),
    onSuccess: async () => {
      setSelectedId("");
      await Promise.all([
  ...
  const remove = useMutation({
    mutationFn: (userId: string) => courseApi.removeReviewer(courseId, userId),
