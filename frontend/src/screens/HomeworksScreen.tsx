...
function CreateHomeworkForm({
  ...
  const extractCriteria = useMutation({
    mutationFn: () => courseApi.parseCriteria({ title: form.title, taskUrl: form.task_url, file: taskFile }),
    onSuccess: (extracted) => {
      if (!extracted.length) {
        setTaskFileError("Модель не нашла критериев в задании");
        return;
      }
      setCriteria(extracted.map((item) => ({
        title: item.title,
        description: item.description ?? "",
        max_score: Number.parseFloat(String(item.max_score)),
      })));
      setTaskFileError("");
    },
  });
  ...
  function submit(event: FormEvent) {
    ...
    if (criteria.some((criterion) => !Number.isFinite(criterion.max_score) || criterion.max_score < 0)) {
      setTaskFileError("Баллы по критериям должны быть неотрицательными числами");
      return;
    }
    setTaskFileError("");
    create.mutate();
      ...
      <div className="sm:col-span-2 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            disabled={extractCriteria.isPending || (!taskFile && !form.task_url.trim())}
            onClick={() => {
              setTaskFileError("");
              extractCriteria.mutate();
            }}
          >
            {extractCriteria.isPending ? "Извлекаем…" : "Получить критерии из модели"}
          </button>
        </div>
        {extractCriteria.error && <p className="text-xs text-danger">{extractCriteria.error.message}</p>}
