...
export const courseApi = {
  ...
  parseCriteria: async (payload: { title: string; taskUrl: string; file?: File | null }) => {
    if (!payload.file) {
      return api<Assignment["criteria"]>("/api/methodist/tasks/parse-criteria", {
        method: "POST",
        body: JSON.stringify({ title: payload.title, task_url: payload.taskUrl }),
      });
    }
    const form = new FormData();
    form.append("title", payload.title);
    form.append("file", payload.file);
    const response = await fetch(`${apiBaseUrl()}/api/methodist/tasks/parse-criteria`, {
      method: "POST",
      headers: authToken ? { Authorization: `Bearer ${authToken}` } : {},
      body: form,
    });
    if (!response.ok) throw await requestError(response);
    return response.json() as Promise<Assignment["criteria"]>;
  },
...
export const homeworkApi = {
  ...
  notifyBehindReviewers: (id: number) =>
    api<{ notified_count: number; reviewers: Array<{ reviewer: string; pending_count: number }>; lagging_reviewers: Array<{ reviewer: string; pending_count: number }> }>(
      `/api/methodist/assignments/${id}/notify-behind-reviewers`,
