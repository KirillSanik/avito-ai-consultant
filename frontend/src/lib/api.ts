...
export async function api<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new Error(
      "Не удалось подключиться к серверу. Проверьте, что FastAPI запущен на http://localhost:8000.",
    );
  }
...
export const courseApi = {
  list: (asReviewer = false) =>
    api<Course[]>(asReviewer ? "/courses?as_reviewer=true" : "/courses"),
  create: (payload: CourseCreate) =>
    api<Course>("/courses", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  updateDescription: (courseId: string, description: string) =>
    api<Course>(`/courses/${courseId}`, {
      method: "PATCH",
      body: JSON.stringify({ description }),
    }),
  homeworks: (courseId: string) => api<HomeworkListItem[]>(`/courses/${courseId}/homeworks`),
  createHomework: (courseId: string, payload: HomeworkCreate) => {
    const form = new FormData();
    ...
    return apiForm<HomeworkListItem>(`/courses/${courseId}/homeworks`, form);
  },
  reviewerCatalog: () => api<User[]>("/api/reviewers"),
  reviewers: (courseId: string | number) =>
    api<CourseReviewer[]>(`/api/courses/${courseId}/reviewers`),
  addReviewer: (courseId: string | number, reviewerId: string) =>
    api<CourseReviewer>(`/api/courses/${courseId}/reviewers`, {
      method: "POST",
      body: JSON.stringify({ reviewer_id: reviewerId }),
    }),
  removeReviewer: (courseId: string | number, reviewerId: string) =>
    api(`/api/courses/${courseId}/reviewers/${reviewerId}`, { method: "DELETE" }),
  exportXlsx: (courseId: string) =>
    downloadBlob(`/api/courses/${courseId}/export.xlsx`, `course-${courseId}.xlsx`),
  importReviewers: (courseId: string, file: File, confirm = false) =>
...
export const homeworkApi = {
  get: (id: string, asReviewer = false) =>
    api<Assignment>(`/api/assignments/${id}${asReviewer ? "?as_reviewer=true" : ""}`),
  getSubmission: (id: string | number) => api<Submission>(`/api/submissions/${id}`),
