...
- [ ] T029 Perform manual end-to-end run per quickstart.md §5 against a real LLM server (vLLM/Triton) on a real public repository: валидный `AIAssessmentResult` (ровно 5 полей), время < 5 минут для ≤20 поддерживаемых файлов (SC-001), отсутствие следов клона после прогона (SC-004), стабильность `status` при повторном прогоне (SC-005); пройтись по негативной таблице quickstart.md §6 (требует доступного LLM-сервера и сети)
  - **Статус (недоступно)**: в среде нет реального LLM-сервера (порты 8000/8001/8080/3000/11434 закрыты, vLLM/Triton не развёрнуты), поэтому ручное E2E против живой модели не выполнено. E2E с stub-сервером Structured Output (127.0.0.1:8199) на реальном публичном репозитории (pallets/markupsafe) пройден: вердикт ровно 5 полей, `response_format=json_schema`, `temperature=0`, полный код 16 поддерживаемых файлов без усечения, temp-каталоги чисты, время ~1.3 c (SC-001); негативный сценарий (репозиторий только с изображениями) → `CodeAggregationError`, exit=1, без следов клона (SC-004); стабильность `status` при повторных прогонах подтверждена детерминированным stub (temperature=0, SC-005). Осталось: финальный прогон на реальной модели (vLLM/Triton).
