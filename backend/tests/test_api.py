...
def test_telegram_handle_alias_is_normalized_and_linked(client: TestClient) -> None:
    registered = client.post(
        "/api/auth/register",
        json={
            "login": "telegram_reviewer",
            "password": "safe-password",
            "first_name": "Telegram",
            "last_name": "Reviewer",
            "telegram_username": "@ArtemChebykin",
            "role": "reviewer",
        },
    )

    assert registered.status_code == 201
    assert registered.json()["user"]["telegram_username"] == "artemchebykin"

    linked = client.post(
        "/api/telegram/webhook",
        json={"message": {"from": {"id": 987654, "username": "ARTEMCHEBYKIN"}}},
    )

    assert linked.status_code == 200
    assert linked.json()["status"] == "linked"
    with SessionLocal() as db:
        user = db.scalar(select(User).where(User.login == "telegram_reviewer"))
        assert user is not None
        assert user.telegram_username == "artemchebykin"
        assert user.tg_id == 987654
