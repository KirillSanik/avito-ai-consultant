...
class SubmissionParser:
    supported_extensions = frozenset({".py", ".ipynb", ".sql", ".sh", ".go", ".md", ".txt", ".docx", ".pdf", ".xlsx", ".js", ".ts"})
    ...
    def _parse_file(self, source: Path) -> dict:
        ...
        if suffix == ".ipynb":
            return {"text": parse_document(source.read_bytes(), f"{source.name}.txt").text, "tables": [], "image_count": 0}
        return {"text": parse_document(source.read_bytes(), f"{source.name}.txt").text, "tables": [], "image_count": 0}
