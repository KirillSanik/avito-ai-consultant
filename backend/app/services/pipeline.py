...
from .contracts import EvaluationReport, ReviewResponse, TaskRubric
...
class EvaluationPipeline:
    ...
    async def run(self, submission_id: str, source_type: str, source: str, rubric: TaskRubric) -> ReviewResponse:
        if source_type == "file":
            ...
            ai_assessment = await self.llm.assess_ai_origin(rubric.full_instructions, submission.file_tree, [], submission.raw_text)
        else:
            repository = await self.cloner.clone(source)
            try:
                submission, ai_assessment = await asyncio.gather(
                    asyncio.to_thread(self.submissions.build_from_local_repository, repository, submission_id, rubric.task_id),
                    self.detector.analyze(rubric.full_instructions, repository),
                )
            finally:
                shutil.rmtree(repository.parent, ignore_errors=True)
        results = await grade_submission(self.llm, rubric, submission)
