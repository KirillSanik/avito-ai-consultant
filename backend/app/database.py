...
def ensure_schema() -> None:
    ...
    with engine.begin() as conn:
        ...
        if engine.dialect.name == "postgresql" and "submissions" in tables:
            score_column = next((column for column in inspector.get_columns("submissions") if column["name"] == "score"), None)
            if score_column is not None and "INTEGER" in str(score_column["type"]).upper():
                conn.execute(text("ALTER TABLE submissions ALTER COLUMN score TYPE DOUBLE PRECISION USING score::double precision"))
