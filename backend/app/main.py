...
def serialize_course_reviewer(link: CourseReviewer) -> CourseReviewerOut:
    return CourseReviewerOut(
        ...
        telegram_username=link.user.telegram_username or link.user.telegram,
        tg_id=link.user.tg_id,
    )
...
def register(payload: UserCreate, db: Session = Depends(get_db)) -> AuthResponse:
    ...
    telegram = normalize_telegram_username(
        payload.telegram_username or payload.tg_handle or payload.telegram
    )
    if not telegram:
...
def update_auth_me(
    ...
    if any(value is not None for value in (payload.telegram, payload.telegram_username, payload.tg_handle)):
        clean_username = normalize_telegram_username(
            payload.telegram_username or payload.tg_handle or payload.telegram
        )
        current_user.telegram = clean_username
        current_user.telegram_username = clean_username or None
    db.commit()
...
def telegram_webhook(payload: dict, db: Session = Depends(get_db)) -> dict[str, object]:
    ...
    if not username:
        return {"status": "unlinked", "tg_id": int(sender["id"])}
    user = db.scalar(
        select(User).where(func.lower(User.telegram_username) == username)
    )
    if user is None:
        user = db.scalar(select(User).where(func.lower(User.telegram) == username))
    if user is None:
        return {"status": "unlinked", "username": username, "tg_id": int(sender["id"])}
    user.telegram_username = username
    user.telegram = username
    user.tg_id = int(sender["id"])
    for link in db.scalars(select(AssignmentReviewer).where(AssignmentReviewer.user_id == user.id)).all():
        link.telegram_username = username
        link.telegram = username
        link.tg_id = user.tg_id
    db.commit()
    logger.info(f"Successfully linked Telegram chat_id {user.tg_id} to user {username}")
    return {"status": "linked", "user_id": user.id, "tg_id": user.tg_id}
...
def submit_student_file(
    ...
    if suffix not in SUPPORTED_TASK_EXTENSIONS:
        raise HTTPException(status_code=422, detail="Поддерживаются PDF, DOCX, XLSX, MD и TXT")
...
async def create_assignment(
    ...
    if task_file is not None:
        await _attach_task_file(assignment, task_file)
    else:
        try:
            extracted_url_text = extract_url_text(assignment.task_url)
            if extracted_url_text:
                assignment.task_text = extracted_url_text
        except Exception:
            logger.exception("task.url_parse_failed assignment_id=%s url=%s", assignment.id, assignment.task_url)
...
@app.post("/api/methodist/tasks/parse-criteria")
async def parse_task_criteria(
    request: Request,
    _: User = Depends(require_methodist),
    ...
    title = "Новое задание"
    task_url = ""
    has_file = "multipart/form-data" in content_type
    if "multipart/form-data" in content_type:
        form = await request.form()
        title = str(form.get("title") or title)
        task_file = form.get("file")
        logger.info(
            "task.criteria_parse_request has_file=%s has_url=%s title=%s filename=%s",
            task_file is not None,
            False,
            title,
            getattr(task_file, "filename", None),
        )
        if task_file is None or not hasattr(task_file, "read") or not hasattr(task_file, "filename"):
            raise HTTPException(status_code=422, detail="Прикрепите файл задания")
        content = await task_file.read()
        try:
            text = parse_document(content, task_file.filename).text
        except Exception as exc:
        ...
        logger.info(
            "task.criteria_parse_request has_file=%s has_url=%s title=%s",
            has_file,
            bool(task_url),
            title,
        )
        if task_url:
            try:
                text = extract_url_text(task_url)
            except Exception as exc:
                raise HTTPException(status_code=422, detail=f"Не удалось загрузить задание: {exc}") from exc
    if len(text.strip()) < 20:
        logger.warning(
            "task.criteria_parse_insufficient_text title=%s source=%s raw_content_length=%s preview=%r",
            title,
            task_url or "file-or-text",
            len(text.strip()),
            text[:200],
        )
        raise HTTPException(status_code=422, detail="Could not extract text from document/URL")
    logger.info(
        "task.criteria_parse_content title=%s raw_content_length=%s preview=%r",
        title,
        len(text),
        text[:200],
    )
    fallback = fallback_rubric("preview", title, text)
    try:
        rubric = await LLMService(PipelineSettings.from_environment()).parse_rubric("preview", title, text, fallback)
    except Exception:
        logger.exception("task.criteria_parse_llm_failed title=%s", title)
        rubric = fallback
    criteria = [
        {
            "title": criterion.name,
            "description": criterion.description,
            "max_score": float(criterion.max_points),
        }
        for criterion in rubric.criteria
    ]
    if not criteria:
        logger.error("task.criteria_parse_empty_result title=%s", title)
        raise HTTPException(status_code=422, detail="Could not derive criteria from document/URL")
    logger.info("task.criteria_parse_result title=%s criteria=%s", title, criteria)
    return criteria
...
async def _attach_task_file(assignment: Assignment, file: UploadFile) -> None:
    suffix = Path(file.filename or "").suffix.lower()
    if suffix not in SUPPORTED_TASK_EXTENSIONS:
        raise HTTPException(status_code=422, detail="Поддерживаются PDF, DOCX, XLSX, Markdown и TXT")
    content = file.file.read()
    if not content:
    ...
    destination.write_bytes(content)
    fallback = fallback_rubric(str(assignment.id), assignment.title, parsed_document.text, assignment.criteria)
    try:
        rubric = await LLMService(PipelineSettings.from_environment()).parse_rubric(
            str(assignment.id), assignment.title, parsed_document.text, fallback
        )
    ...
    assignment.criteria = [
        {"title": criterion.name, "description": criterion.description, "max_score": float(criterion.max_points)}
        for criterion in rubric.criteria
    ]
...
def attach_homework_reviewer(
    ...
    if by_telegram is not None:
        ...
        by_telegram.telegram_username = user.telegram_username or user.telegram
        by_telegram.tg_id = user.tg_id
        return by_telegram
    reviewer = AssignmentReviewer(
        ...
        telegram_username=user.telegram_username or user.telegram,
        tg_id=user.tg_id,
...
@app.get("/api/reviewer/submissions/{submission_id}/evaluation")
def get_reviewer_submission_evaluation(
    submission_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> dict[str, object]:
    ...
    if submission is None:
        raise HTTPException(status_code=404, detail="Submission not found")
    get_assignment_for_user(submission.assignment_id, current_user, db)
    if current_user.role == "reviewer" and submission.reviewer_user_id != current_user.id:
    ...
    if evaluation is None or evaluation.review_json is None:
    ...
    report = evaluation.review_json
    results = report.get("criterion_results", []) if isinstance(report, dict) else []
    by_name = {
        str(item.get("criterion_name")): item
        for item in results
        if isinstance(item, dict) and item.get("criterion_name")
    }
    by_id = {
    ...
    criteria_breakdown = []
    for index, criterion in enumerate(assignment.criteria):
        item = by_id.get(str(index)) or by_name.get(str(criterion.get("title", ""))) or {}
        max_score = float(criterion.get("max_score", 0))
        criteria_breakdown.append(
            {
                "id": str(index),
                "title": str(criterion.get("title", "Criterion")),
                "description": str(criterion.get("description", "")),
                "max_score": max_score,
                "awarded_score": min(max(float(item.get("assigned_score", 0) or 0), 0), max_score),
                "reasoning": str(item.get("reasoning", "")),
            }
        )
    max_total_score = float(evaluation.max_total_score or sum(item["max_score"] for item in criteria_breakdown))
    total_score = float(evaluation.total_score if evaluation.total_score is not None else sum(item["awarded_score"] for item in criteria_breakdown))
    overall_comment = str(report.get("summary_feedback", submission.summary or ""))
    return {
        "total_score": total_score,
        "max_total_score": max_total_score,
        "criteria_breakdown": criteria_breakdown,
        "overall_comment": overall_comment,
        "ai_comment": overall_comment,
    }
...
@app.post("/api/methodist/assignments/{assignment_id}/notify-behind-reviewers")
async def notify_behind_reviewers(
    assignment_id: int,
    db: Session = Depends(get_db),
    _: User = Depends(require_methodist),
) -> dict[str, object]:
    ...
    reviewers = db.scalars(
        select(AssignmentReviewer).where(AssignmentReviewer.assignment_id == assignment_id)
    ).all()
    ...
    for reviewer in reviewers:
        assigned = [
            submission for submission in assignment.submissions
            if reviewer.user_id is not None and submission.reviewer_user_id == reviewer.user_id
        ]
        total = len(assigned) if reviewer.user_id is not None else reviewer.total
        checked = sum(item.status == "reviewed" for item in assigned) if reviewer.user_id is not None else reviewer.checked
        pending = max(0, total - checked)
        if total <= 0 or checked >= total:
            continue
        user = reviewer.user
        linked_chat_id = (user.tg_id if user is not None else None) or reviewer.tg_id
        username = normalize_telegram_username(
            (user.telegram_username if user is not None else None)
            or reviewer.telegram_username
            or (user.telegram if user is not None else None)
            or reviewer.telegram
        )
        logger.info(
            f"Checking reviewer {user.id if user is not None else reviewer.user_id} "
            f"({user.login if user is not None else reviewer.name}): "
            f"tg_id={linked_chat_id}, tg_username={username}"
        )
        if linked_chat_id is None and not username:
            logger.warning(
                "behind_reviewers.skipped_unlinked assignment_id=%s reviewer=%s",
                assignment_id,
                reviewer.name,
            )
            continue
        keys = [str(reviewer.user_id or ""), reviewer.name, reviewer.telegram, username]
        if user is not None:
            keys.extend([str(user.id), user.login, user.telegram.removeprefix("@")])
        chat_id = str(linked_chat_id) if linked_chat_id is not None else next(
            (str(mapping[key]) for key in keys if key and isinstance(mapping, dict) and mapping.get(key) is not None),
            None,
        )
        if chat_id is None:
            raw_id = reviewer.telegram.strip()
            chat_id = raw_id if raw_id.lstrip("-").isdigit() else None
        flagged.append({"reviewer": reviewer.name, "pending_count": pending, "chat_id": chat_id, "username": username or None})

    if not flagged:
        return {"notified_count": 0, "reviewers": [], "lagging_reviewers": []}
    if not notify_url or not api_key:
        logger.warning("behind_reviewers.notifications_not_configured assignment_id=%s flagged=%s", assignment_id, len(flagged))
        return {"notified_count": 0, "reviewers": [], "lagging_reviewers": [{"reviewer": item["reviewer"], "pending_count": item["pending_count"]} for item in flagged]}

    async def send(item: dict[str, object]) -> dict[str, object] | None:
        if not item["chat_id"] and not item["username"]:
            logger.warning("behind_reviewers.missing_chat_id assignment_id=%s reviewer=%s", assignment_id, item["reviewer"])
            return None
        payload = {
            "chat_id": item["chat_id"],
            "username": item["username"],
            "message": (
                f"Напоминание по проверке ДЗ «{assignment.title}»: у вас осталось "
                f"{item['pending_count']} непроверенных работ. Пожалуйста, завершите проверку."
            ),
        }
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.post(endpoint, json=payload, headers={"X-API-Key": api_key})
                response.raise_for_status()
            logger.info(
                "behind_reviewers.notification_sent assignment_id=%s reviewer=%s status=%s",
                assignment_id,
                item["reviewer"],
                response.status_code,
            )
            return {"reviewer": item["reviewer"], "pending_count": item["pending_count"]}
        except httpx.HTTPError as exc:
            response = getattr(exc, "response", None)
            logger.error(
                "behind_reviewers.notification_failed assignment_id=%s reviewer=%s "
                "status=%s body=%s error=%s",
                assignment_id,
                item["reviewer"],
    ...
    results = await asyncio.gather(*(send(item) for item in flagged))
    notified = [item for item in results if item is not None]
    return {"notified_count": len(notified), "reviewers": notified, "lagging_reviewers": [{"reviewer": item["reviewer"], "pending_count": item["pending_count"]} for item in flagged]}
