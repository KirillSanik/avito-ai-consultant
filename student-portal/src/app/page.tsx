            ...
            <section className="card p-6">
              ...
              {homework.data.submission && (
                <div className="my-4 rounded-lg bg-secondary p-3 text-sm">
                  ...
                  {homework.data.submission.criterion_scores?.length ? (
                    <div className="mt-3 space-y-2 border-t border-border pt-3">
                      {homework.data.submission.criterion_scores.map((criterion) => (
                        <div key={criterion.criterion} className="text-xs text-muted">
                          <span className="font-medium text-foreground">{criterion.criterion}: </span>
                          {criterion.score}/{criterion.max_score}
                          {criterion.comment ? <span className="block mt-1">{criterion.comment}</span> : null}
                        </div>
                      ))}
                    </div>
                  ) : null}
